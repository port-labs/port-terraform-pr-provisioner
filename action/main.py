...
from clients.port import (
    create_port_entity,
    report_action_status,
    add_action_log_message,
)
...
def lambda_handler(event, context):
    ...
    action_identifier = req_body["action"]
    run_id = req_body["context"]["runId"]
    add_action_log_message(
        run_id=run_id, message=f"🧪 Starting action - {action_identifier}"
    )
    ...
    if action_identifier == "add_document_db":
        ...
        if status == "FAILURE":
            return {"body": {"message": "Encountered error"}, "statusCode": 500}
        ...
        if status == "FAILURE":
            return {"body": {"message": "Encountered error"}, "statusCode": 500}
    result = report_action_status(
        run_id,
        "IN_PROGRESS",
        f"Created new PR for the requested resource: {pr_url}",
        [pr_url],
    )

    logger.info(f"Got: {result}")
    add_action_log_message(
        run_id, "🚥 Continue this action by reviewing and merging the PR"
    )

    return {"body": {"message": "PR created"}, "statusCode": 200}
